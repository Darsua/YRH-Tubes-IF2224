...
shared_ptr<ParseNode> Parser::parseTypeDeclaration() {
    ...
    } while (match(IDENTIFIER) && !matchKeyword("variabel") && 
             !matchKeyword("prosedur") && !matchKeyword("fungsi") && !matchKeyword("mulai"));
...
shared_ptr<ParseNode> Parser::parseVarDeclaration() {
    ...
    } while (match(IDENTIFIER) && !matchKeyword("prosedur") && 
             !matchKeyword("fungsi") && !matchKeyword("mulai") &&
             !matchKeyword("konstanta") && !matchKeyword("tipe"));
...
// Grammar: <assignment-statement> -> IDENTIFIER (DOT IDENTIFIER | LBRACKET <expression> RBRACKET)* ASSIGN_OPERATOR <expression>
...
// Grammar: <factor> -> IDENTIFIER (DOT IDENTIFIER | LBRACKET <expression> RBRACKET | LPARENTHESIS <parameter-list> RPARENTHESIS)* | NUMBER | STRING_LITERAL | CHAR_LITERAL | LPARENTHESIS <expression> RPARENTHESIS | LOGICAL_OPERATOR(tidak) <factor> | (ARITHMETIC_OPERATOR(+|-)) <factor>
